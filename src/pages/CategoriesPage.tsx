...
export function CategoriesPage() {
  ...
  async function save() {
    ...
    await saveCategory(current ? { ...current, name: name.trim() } : { id: crypto.randomUUID(), name: name.trim(), icon: '◌', color: palette[categories.length % palette.length], createdAt: new Date().toISOString(), sortOrder: categories.length })
  ...
  return <><PageHeader title="Kategorien" />
    <main className="page"><div className="page-title-row"><div><span className="eyebrow">Deine Bereiche</span><h2>Alles an seinem Platz</h2></div><button className="compact-add" onClick={() => { setEditing('new'); setName('') }}><Plus /> Neu</button></div>
      {editing && <div className="edit-panel"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Kategoriename" autoFocus /><button onClick={save}>Speichern</button><button onClick={() => setEditing(null)}>Abbrechen</button></div>}
      <div className="category-list">{categories.length ? categories.map((category, index) => {
        ...
        return <article className="category-card" key={category.id}>
          <button className="category-open" onClick={() => navigate(`/category/${category.id}`)}>
            <span className="category-icon" style={{ background: `${category.color}1f`, color: category.color }}>{category.icon}</span>
          ...
          <div className="category-actions"><button disabled={index === 0} onClick={() => move(index, -1)} aria-label={`${category.name} nach oben`}><ChevronUp /></button><button disabled={index === categories.length - 1} onClick={() => move(index, 1)} aria-label={`${category.name} nach unten`}><ChevronDown /></button><button onClick={() => { setEditing(category.id); setName(category.name) }} aria-label={`${category.name} bearbeiten`}><Pencil /></button><button onClick={() => remove(category.id)} aria-label={`${category.name} löschen`}><Trash2 /></button></div>
        </article>
