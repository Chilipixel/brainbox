...
export function CategoryDetailPage() {
  ...
  return <><PageHeader title={category?.name ?? 'Kategorie'} back />
    <main className="page"><div className="detail-hero"><span className="large-category-icon" style={{ background: `${category?.color}20`, color: category?.color }} aria-hidden="true">{categoryEmoji(category) || '·'}</span><div><span className="eyebrow">Kategorie</span><h2>{category?.name}</h2><p>{ownTasks.length} offene Aufgaben</p></div></div>
      <label className="sort-control">Sortierung<select value={sort} onChange={(e) => setSort(e.target.value)}><option value="priority">Priorität</option><option value="duration">Dauer</option><option value="energyAsc">Energie: wenig bis hoch</option><option value="energyDesc">Energie: hoch bis wenig</option><option value="alphabetical">Alphabetisch</option><option value="dueDate">Fälligkeit</option></select></label>
